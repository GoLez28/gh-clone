...
namespace Upbeat.Elements {
    class Song {
        public static void Draw (MenuDraw_SongSelector item, int i, float Y, float songHeight, float songSelectionStart, float songSelectionEnd, int selectedTarget, float textMarginX, float textMarginY, float scalef) {
            ...
            float textHeight = (Upbeat.Draw.font.Height) * scalef * 0.7f;
            ...
            float width = Upbeat.Draw.GetWidthString(name, textScale);
            ...
            Upbeat.Draw.DrawString(name, textX, textY, textSquish, white, alignCorner, 0, songSelectionEnd);
            if (SongList.sorting != SortType.Name || SongList.sorting != SortType.Artist) {
                ...
                width = Upbeat.Draw.GetWidthString(subInfo, textScaleSmol);
                Upbeat.Draw.DrawString(subInfo, songSelectionEnd - width - textMarginX, textY + textHeight * 0.8f, textScaleSmol, softWhite, alignCorner);
            }
            Upbeat.Draw.DrawString(info.Artist, textX + textMarginX, textY + textHeight * 0.8f, textScaleSmol, softWhite, alignCorner, 0, songSelectionEnd); //TextH prev = 0.9f
        }
    }
}
