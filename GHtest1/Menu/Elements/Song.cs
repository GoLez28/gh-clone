...
namespace Upbeat.Elements {
    class Song {
        public static void Draw (MenuDraw_SongSelector item, int i, float Y, float songHeight, float songSelectionStart, float songSelectionEnd, int selectedTarget, float textMarginX, float textMarginY, float scalef) {
            ...
            Upbeat.Draw.Text.DrawString(name, textX, textY, textSquish, white, alignCorner, 0, songSelectionEnd);
            if (SongList.sorting != SortType.Name || SongList.sorting != SortType.Artist) {
                ...
                width = Upbeat.Draw.Text.GetWidthString(subInfo, textScaleSmol);
                Upbeat.Draw.Text.DrawString(subInfo, songSelectionEnd - width - textMarginX, textY + textHeight * 0.8f, textScaleSmol, softWhite, alignCorner);
            }
            Upbeat.Draw.Text.DrawString(info.Artist, textX + textMarginX, textY + textHeight * 0.8f, textScaleSmol, softWhite, alignCorner, 0, songSelectionEnd); //TextH prev = 0.9f
        }
    }
}
